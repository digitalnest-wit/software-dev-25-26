# Shopping Cart System (BUGGY VERSION)
...
def calculate_subtotal(cart):
    subtotal = 0
    for item in cart:
        subtotal += item["price"] + item["quantity"]  # Bug: should multiply, not add
    return subtotal

def apply_discount(subtotal, discount_percent):
    discount_amount = subtotal * discount_percent / 100
    final_price = subtotal + discount_amount  # Bug: should subtract discount
    return final_price

def calculate_tax(amount, tax_rate):
    tax = amount * tax_rate
    print(tax)  # Bug: prints instead of returning

def calculate_total(subtotal, discount, tax_rate):
    after_discount = apply_discount(discount, subtotal)  # Bug: wrong parameter order
...
def display_receipt(cart, subtotal, discount, total):
    ...
    discount_amount = 0  # Bug: variable scope - can't access actual discount amount
...
while True:
    ...
    if choice == "1":
        ...
        shopping_cart = add_item(shopping_cart, name, price, qty)  # Bug: function doesn't return
        print(f"{name} added to cart")
    
    elif choice == "2":
        name = input("Item name to remove: ")
        result = remove_item(shopping_cart, name)
        if result:  # Bug: function doesn't return False properly
